// Finding the git checkouts a fragment of a directory name could mean.
//
// This exists for one bit of friction: an operation spans several repos, and typing
...
var DefaultScanRoots = []string{"~/dev", "~/src", "~/code"}
...
var DefaultScanSkip = []string{"node_modules", "vendor", "target", "Library"}

// ScanOptions is where and how far to search. The zero value is usable and means the
// defaults above.
...
func (o ScanOptions) withDefaults() ScanOptions {
	if len(o.Roots) == 0 {
		o.Roots = DefaultScanRoots
	}

	if o.MaxDepth <= 0 {
		o.MaxDepth = DefaultScanDepth
	}

	if len(o.Skip) == 0 {
		o.Skip = DefaultScanSkip
	}
...
func (o ScanOptions) skips(name string) bool { return slices.Contains(o.Skip, name) }
...
func Scan(opts ScanOptions) []Candidate {
	...
	for _, root := range ScanRoots(opts.Roots) {
...
func scanRoot(root string, opts ScanOptions) []Candidate {
	var out []Candidate
