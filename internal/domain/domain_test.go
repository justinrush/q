...
func TestBadgeHelpers(t *testing.T) {
	...
	mission.Badges = mission.WithBadge(BadgeStale, "")
	if !mission.HasBadge(BadgeStale) {
	...
	mission.Badges = mission.WithBadge(BadgeStale, "5m")
	if len(mission.Badges) != 1 {
		t.Fatalf("len(Badges) = %d, want 1", len(mission.Badges))
	}

	if mission.Badges[0].Detail != "5m" {
		t.Errorf("Detail = %q, want %q", mission.Badges[0].Detail, "5m")
	}

	mission.Badges = mission.WithBadge(BadgeAPIError, "rate_limit")
	mission.Badges = mission.WithoutBadge(BadgeStale)

	if mission.HasBadge(BadgeStale) {
		t.Error("badge not removed")
	}

	if !mission.HasBadge(BadgeAPIError) {
		t.Error("removing one badge dropped another")
	}
}

func TestWithoutBadgeIsANoopWhenAbsent(t *testing.T) {
	mission := Mission{Badges: []Badge{{Kind: BadgeAPIError}}}

	got := mission.WithoutBadge(BadgeStale)
	if len(got) != 1 {
...
func TestLaunchedAndResumable(t *testing.T) {
	...
	mission.AgentSessionID = "abc"

	if !mission.Launched() || !mission.Resumable() {
...
func TestWorktreesSortedByRepoName(t *testing.T) {
	mission := Mission{Work: map[string]RepoWork{
