// Package domain holds q's entity types and the pure logic over them.
//
// It has no dependencies outside the standard library, deliberately: the state
// machine and the persistence layer both build on these types, and both are
// tested without touching a filesystem, a terminal, or a subprocess.
package domain
