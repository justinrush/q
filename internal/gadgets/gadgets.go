// Package binpath resolves absolute paths to the external programs q
// drives.
//
...
// Tool names an external program q depends on.
type Tool string
...
// All lists every tool, in the order q doctor reports them.
var All = []Tool{Git, Tmux, Claude, Codex, OsaScript}

// DefaultRequired lists the tools q cannot function at all without.
//
...
// callers that need it add it to Options.Required.
var DefaultRequired = []Tool{Git, Tmux}

// GhosttyApp is the app bundle q controls through its AppleScript API.
const GhosttyApp = "/Applications/Ghostty.app"

// From derives resolver options from the user's settings.
//
...
func From(s settings.Settings) Options {
	overrides := map[Tool]string{}

	for name, path := range s.Tools {
		overrides[Tool(name)] = path
	}
	...
	return Options{Overrides: overrides, Required: RequiredFor(s)}
}

// RequiredFor lists what a configuration cannot start without.
//
...
func RequiredFor(s settings.Settings) []Tool {
	tools := []Tool{Git, Tmux}

	if s.Terminal.Mode == settings.TerminalGhostty {
		tools = append(tools, OsaScript)
	}
...
var fallbacks = map[Tool][]string{
	...
	Codex:     {"~/.nvm/versions/node/*/bin/codex", "/opt/homebrew/bin/codex"},
	OsaScript: {"/usr/bin/osascript"},
}

// Resolver caches tool paths for the lifetime of a process. It is safe for
// concurrent use.
type Resolver struct {
	mu        sync.Mutex
	cache     map[Tool]string
	errors    map[Tool]error
	overrides map[Tool]string
	required  []Tool
}

// New returns a Resolver honoring the given options.
...
func (r *Resolver) MustPath(t Tool) string {
	p, err := r.Path(t)
	if err != nil {
		return ""
	}
...
func resolve(t Tool, configured string) (string, error) {
	source := t.EnvVar()
...
func firstFallback(t Tool) string {
