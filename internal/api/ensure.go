...
func connect(ctx context.Context, dirs paths.Dirs) (*Client, error) {
	c, err := OpenClient(dirs)
	if err != nil {
		return nil, err
	}

	if _, err := c.Health(ctx); err != nil {
		return nil, fmt.Errorf("%w: handle present but daemon unreachable: %w", ErrNoDaemon, err)
	}
...
func spawn(dirs paths.Dirs) error {
	...
	if err := RemoveHandle(dirs.DaemonFile()); err != nil {
		return err
	}
...
func Stop(dirs paths.Dirs) error {
	...
	if err != nil {
		return RemoveHandle(dirs.DaemonFile())
	}
