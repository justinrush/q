...
func newTestOpener() (*Ghostty, *runner.Fake) {
	fake := runner.NewFake()

	return NewGhostty(osaScriptBin, fake), fake
}
...
// A Command template is expanded without a shell, so a mission directory
// containing a space cannot change what runs.
func TestCommandExpandsTheTemplate(t *testing.T) {
	fake := runner.NewFake()

	opener, err := NewCommand([]string{"wezterm", "start", "--cwd", "{dir}", "--", "{argv}"}, fake)
	if err != nil {
...
func TestCommandRejectsATemplateWithNoCommandPlaceholder(t *testing.T) {
	if _, err := NewCommand([]string{"wezterm", "start", "--cwd", "{dir}"}, runner.NewFake()); err == nil {
		t.Fatal("expected an error for a template with no {argv} or {cmd}")
	}
}

// Manual opens nothing and says what to run instead.
func TestManualReportsTheCommandToRunByHand(t *testing.T) {
	fake := runner.NewFake()
	opener := NewManual()
