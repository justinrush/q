...
// Heal reports what claude's own registry says about each of the given missions.
//
// It implements [mission.Healer]. A missing registry, an unreadable entry, or a
// mission claude has never heard of all yield no reading rather than an error:
// the registry is a best-effort signal and hooks remain q's primary source.
func (r *Registry) Heal(_ context.Context, missions []mission.Mission) (map[mission.MissionID]mission.Reading, error) {
	dir := r.dir
	if dir == "" {
		var err error
	...
	bySession := BySessionID(sessions)
	out := make(map[mission.MissionID]mission.Reading, len(missions))

	for _, ms := range missions {
		if ms.Tool != mission.ToolClaude || ms.AgentSessionID == "" {
			continue
		}

		session, ok := bySession[ms.AgentSessionID]
		if !ok {
			continue
		}
...
// activityOf maps a registry status onto q's vocabulary.
func activityOf(status string) mission.Activity {
	switch status {
	case StatusAwaiting:
...
// Registry reads claude's live session registry.
//
...
// NewRegistry returns a healer reading the registry at dir. An empty dir uses
// [DefaultDir].
