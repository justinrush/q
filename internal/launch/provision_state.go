...
type provisionState struct {
	...
	Work      map[string]domain.RepoWork `json:"work,omitempty"`
}
...
func (l *Launcher) prepareMissionDir(operation domain.Operation, mission *domain.Mission) (provisionState, error) {
	name := domain.MissionDirName(operation.Slug, mission.Slug)
	candidates := []string{
		l.dirs.MissionDir(name),
		l.dirs.MissionDir(name + "--" + mission.ID.Short()),
	}

	for index, candidate := range candidates {
		state, available, err := claimMissionDir(candidate, mission.ID)
		if err != nil {
			return provisionState{}, err
		}

		if available {
			mission.MissionDir = candidate
...
func claimMissionDir(dir string, missionID domain.MissionID) (provisionState, bool, error) {
	artifactDir := filepath.Join(dir, loadout.ArtifactDir)
	...
	state := provisionState{MissionID: missionID, Work: make(map[string]domain.RepoWork)}
...
func readProvisionState(path string) (provisionState, error) {
	...
	if state.Work == nil {
		state.Work = make(map[string]domain.RepoWork)
	}
...
func writeProvisionState(missionDir string, state provisionState) error {
	...
	dir := filepath.Join(missionDir, loadout.ArtifactDir)
