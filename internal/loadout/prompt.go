...
// promptData is the template's view of a mission.
...
// ComposePrompt builds the prompt for a mission.
//
...
func ComposePrompt(operation domain.Operation, mission domain.Mission) (string, error) {
	data := promptData{
		OperationName: operation.Name,
		Summary:       strings.TrimSpace(operation.Summary),
		MissionName:   mission.Name,
		Prompt:        strings.TrimSpace(mission.Prompt),
	}
	...
	if err := promptTemplate.Execute(&b, data); err != nil {
		return "", fmt.Errorf("composing prompt for mission %s: %w", mission.ID, err)
	}
