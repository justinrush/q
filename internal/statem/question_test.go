...
func TestStopEndingInAQuestionWaitsOnTheHuman(t *testing.T) {
	res := Reduce(missionIn(domain.StatusActive), hookspec.Payload{
		Event: hookspec.EventStop, SessionID: "sess-1", LastAssistantMessage: codexQuestion,
	}, testNow)

	if res.ProposedStatus != domain.StatusAwaiting {
		t.Errorf("ProposedStatus = %q, want %q", res.ProposedStatus, domain.StatusAwaiting)
	}

	if res.Mission.AgentState != domain.AgentWaiting {
...
func TestQuestionYieldsToBackgroundWorkAndPendingPlans(t *testing.T) {
	...
	planned := missionIn(domain.StatusDebrief)
	planned.PlanPending = true

	res = Reduce(planned, hookspec.Payload{
		Event: hookspec.EventStop, SessionID: "sess-1", LastAssistantMessage: codexQuestion,
	}, testNow)
...
func TestQuestionDoesNotOverwriteALivePermissionPrompt(t *testing.T) {
	blocked := missionIn(domain.StatusAwaiting)
	blocked.WaitingFor = "Bash(rm -rf build)"

	res := Reduce(blocked, hookspec.Payload{
		Event: hookspec.EventStop, SessionID: "sess-1", LastAssistantMessage: codexQuestion,
	}, testNow)
...
func TestAnsweringAQuestionInThePaneReturnsToInProgress(t *testing.T) {
	waiting := missionIn(domain.StatusAwaiting)
	waiting.WaitingFor = "Should I: 1. fix it separately 2. ignore it"

	res := Reduce(waiting, ev(hookspec.EventUserPromptSubmit), testNow)
