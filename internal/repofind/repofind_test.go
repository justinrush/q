...
func TestScanFindsNestedCheckoutsAndPrunesInsideThem(t *testing.T) {
	...
	got := rels(Scan(Options{Roots: []string{root}}))
...
func TestScanSkipsHiddenAndDependencyDirs(t *testing.T) {
	...
	if got := rels(Scan(Options{Roots: []string{root}})); !slices.Equal(got, []string{"kept"}) {
...
func TestScanDeduplicatesOverlappingRoots(t *testing.T) {
	...
	if got := Scan(Options{Roots: []string{root, root + string(filepath.Separator)}}); len(got) != 1 {
...
func TestScanHonorsAConfiguredSkipList(t *testing.T) {
	...
	got := rels(Scan(Options{Roots: []string{root}, Skip: []string{"generated"}}))
...
func TestScanHonorsAConfiguredMaxDepth(t *testing.T) {
	...
	got := rels(Scan(Options{Roots: []string{root}, MaxDepth: 1}))
...
func TestExpandOnlyResolvesThingsThatLookLikePaths(t *testing.T) {
	...
	if got := Expand("~/dev/weave"); got != filepath.Join(home, "dev", "weave") {
		t.Errorf("Expand(~/dev/weave) = %q", got)
	}

	if got := Expand("weave"); got != "" {
		t.Errorf("Expand(weave) = %q, want it left alone", got)
	}

	if got := Expand("/dev/weave"); got != "/dev/weave" {
		t.Errorf("Expand(/dev/weave) = %q", got)
	}
}
...
func TestDirAcceptsAnExistingDirectoryOnly(t *testing.T) {
	...
	if got, ok := Dir(repo); !ok || got != repo {
		t.Errorf("Dir(%s) = %q, %v", repo, got, ok)
	}

	if _, ok := Dir(filepath.Join(root, "missing")); ok {
...
func TestDisplayCollapsesHome(t *testing.T) {
	...
	if got := Display(filepath.Join(home, "dev", "weave")); got != "~/dev/weave" {
		t.Errorf("Display = %q, want ~/dev/weave", got)
	}

	if got := Display("/srv/src/weave"); got != "/srv/src/weave" {
