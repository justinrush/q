...
func MissionRepos(operation Operation, ms Mission) ([]Repo, error) {
	if ms.LaunchReposFrozen {
		return slices.Clone(ms.LaunchRepos), nil
	}

	return CombineRepos(operation.Repos, ms.ExtraRepos)
}
