...
func sampleOperation(name string) Operation {
	return Operation{
		ID:      OperationID("op_aabbccddeeff"),
		Name:    name,
		Slug:    Slug(name),
...
func TestMutatePersistsAcrossReopen(t *testing.T) {
	...
	got, ok := reopened.Snapshot().Operation(OperationID("op_aabbccddeeff"))
	if !ok {
...
func TestSnapshotIsADeepCopy(t *testing.T) {
	...
	if err := store.Mutate("seed", func(s *Snapshot) error {
		...
		s.PutMission(Mission{
			ID:          MissionID("ms_aabbccddeeff"),
			OperationID: OperationID("op_aabbccddeeff"),
			Name:        "mission",
			Status:      StatusBriefing,
			Badges:      []Badge{{Kind: BadgeStale}},
			ExtraRepos:  []Repo{{Name: "mac", Path: "/dev/mac"}},
			LaunchRepos: []Repo{{Name: "weave", Path: "/dev/weave"}},
			Work:        map[string]RepoWork{"weave": {RepoName: "weave"}},
			StartedAt:   &started,
		})
	...
	snap.Missions[0].Work["weave"] = RepoWork{RepoName: "mutated"}
...
func TestOpenRecoversFromBackupWhenPrimaryIsCorrupt(t *testing.T) {
	...
	for _, name := range []string{"First", "Second"} {
		if err := store.Mutate("seed", func(s *Snapshot) error {
			...
			operation.ID = OperationID("op_aabbccddeeff")
	...
	got, ok := recovered.Snapshot().Operation(OperationID("op_aabbccddeeff"))
	if !ok {
...
func TestMutateStampsUpdatedAt(t *testing.T) {
	...
	store, err := Open(dirs, WithStoreClock(func() time.Time { return fixed }))
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
		...
		go func(w int) {
			...
			for i := range 10 {
				_ = store.Mutate("concurrent", func(s *Snapshot) error {
					...
					operation.ID = OperationID("op_" + strings.Repeat("0", 10) + string(rune('a'+w)) + string(rune('a'+i%6)))
					s.PutOperation(operation)
